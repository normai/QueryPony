#region Fileinfo
// file        : http://downtown.trilo.de/svn/queryponydev/trunk/querypony/QueryPonyLib/DbApi/CouchDbBrowser.cs
// id          : 20130616°1601 (20130605°1701)
// summary     : This file stores class 'CouchDbBrowser' to constitute
//                an implementation of IBrowser for CouchDB.
// license     : GNU AGPL v3
...
namespace QueryPonyLib
{
   ...
   internal class CouchDbBrowser : IDbBrowser
   {
      ...
      public string[] GetDatabases()
      {
         ...
         //----------------------------------------------------
         // (sequence 20130723°0951)
         // experimental, first time accessing CouchDB somehow

         ////ICouchServer server = null;
         ////Divan.CouchServer server = null;
         ////server.Host = this.DbClient.ConnSettings.AllDatabaseUrl;
         ////Divan.CouchServer server = new CouchServer(host, port);
         Divan.CouchServer server = new Divan.CouchServer("localhost", (int)Glb.DbSpecs.CouchDefaultPortnum);
//         Divan.CouchServer server = new Divan.CouchServer(this.DbClient.ConnSettings., (int)Glb.DbSpecs.DefaultPortnumberCouch);

         // note : Compare CouchTest.cs line154
         ////bool result = server.GetDatabaseNames().Contains(db.Name);
         ////System.Collections.Generic.List<string> liDbs = null;
                                               ...
                                               )
      {
         ...
         try
         {
            ...
            if (Glb.Debag.ExecuteNo)
            {
               dt.WriteXml(InitLib.SettingsDir + IOBus.Gb.Bricks.PathBkslsh + "debug_couchdb_schema5.xml");
            }
