#region Fileinfo
// file        : http://downtown.trilo.de/svn/queryponydev/trunk/querypony/QueryPonyLib/DbApi/DbClient.cs
// id          : 20130604°0201
...
namespace QueryPonyLib
{
   ...
   /// <remarks>
   /// id : 20130604°0225
   /// note : (note 20130620°1011) In QueryForm.cs::client_Error(), due to refactoring, we get compiler
   ///    error "'ErrorEventArgs' is an ambiguous reference between 'QueryPonyLib.ErrorEventArgs'
   ///    and 'System.IO.ErrorEventArgs'". So we rename this class from ErrorEventArgs to ErrorEventQeArgs.
   /// </remarks>
   ...
   public class ErrorEventQeArgs : EventArgs
   {
      ...
      public ErrorEventQeArgs(string errorMessage, Exception ex) // (refactor 20130620°1011)
      {
   ...
   public abstract class DbClient
   {
      ...
      public event EventHandler<ErrorEventQeArgs> Error; // (refactor 20130620°1011)
      ...
      protected void DoConnect()
      {
         ...
         try
         {
            ...
            // (note 20130713°0933)
            // If the connection is not available, below Open() takes 15 secondes until it
            //  returns unsuccessful. I would like to give it a timeout of e.g. 1 seconds only
            //  for just a ping, as e.g. in method 20130713°0921 ConnectForm.getDatabaseList().
            //  The connection object has a ConnectionTimeout property, but only with a getter.
            //  How can we *set* it? (Solution see sequence 20130713°0934 in
            //  MssqlDbClient::GenerateConnectionString())
            ...
            //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
            // (sequence 20130714°1745) set the database
            ...
            {
               if ((_connSettings.Type != ConnSettingsLib.ConnectionType.Oracle)       // (20130719°0917)
                   && (_connSettings.Type != ConnSettingsLib.ConnectionType.Sqlite)    // this.Database is always 'main' (20130821°1132)
                    )
               {
         ...
         {
            // (debug 20130716°0623) this.Error seems null but should be something
            ...
            // (note 20130614°0723) Open throws e.g.:
            ...
            //
            // - When introducing the PostgreSQL connection (20130720°1202)
            //      exception "Could not load file or assembly 'Mono.Security, Version=2.0.0.0,
            ...
            //
            // - Fixing this by referencing 'Mono.Security.dll'. Plus provisory copying
            //      Mono.Security.dll to QueryPonyGui\bin\x86\Debug. This must be settled a la
            //      single-file-delivery, if the connection finally works. (todo 20130720°1203)
            //
            // - After PostgreSQL Mono.Security.dll was solved  (20130720°1204): Exception
            //     "Failed to establish a connection to 'localhost'.". This is because the
            //     PostgreSQL server was not started.

            // process error message
            ErrorMessage = e.Message;
            OnError(this, new ErrorEventQeArgs(e.Message, e)); // (refactor 20130620°1011)
         }
      ...
      public string ErrorMessage
      {
         ...
         set
         {
            _errorMessage = value; // (breakpoint 20130821°1122)
         }
      }
      ...
      private void DoExecuteBatchesAsync()
      {
         ...
         foreach (string sQuery in _queries)
         {
            ...
            try
            {
               ...
               ReturnResults(dr); // (breakpoint 20130822.2112xx) debugging issue 20130822°2111 'DbClient triggers QueryForm methods'
               dr.Close();
            }
            catch (Exception ex)
            {
               string sDebug = ex.Message;
               ErrorEventQeArgs args = new ErrorEventQeArgs(ex.Message, ex);           // (refactor 20130620°1011)
               OnError(this, args);
      ...
      {
         if (DataReaderAvailable != null) // (breakpoint 20130822.2112xx) debugging issue 20130822°2111 'DbClient triggers QueryForm methods'
         {
      ...
      /// <param name="sender">The sending object</param>
      /// <param name="e">The event object</param>
      protected virtual void OnError(object sender, ErrorEventQeArgs e) // (refactor 20130620°1011)
      {
         //----------------------------------------------------
         // (issue 20130716°0622)
      ...
      /// <remarks>
      /// id : 20130604°0327
      /// todo : Having the identical identifyer for property type and property name is not nice. (todo 20130818°152103)
      /// </remarks>
      public QueryOptions QueryOptions
      {
      ...
      public void Execute(string sQuery)
      {
         ...
         //  separately. Use Regex class, as we need a case insensitive match. (note 20130604°033702)
      ...
      private void ReturnResults(IDataReader dr)
      {
         do
         {
            ...
            result = new DataReaderAvailableEventArgs(dr); // (breakpoint 20130822.211201) debugging issue 20130822°2111 'DbClient triggers QueryForm methods'
